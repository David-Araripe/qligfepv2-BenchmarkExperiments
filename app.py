...
app.layout = html.Div(
    ...
    [
        ...
        html.Div(
            [
                html.Img(id="lig1_img", style={"maxWidth": "35%", "marginRight": "5%", "height": "250px"}),
        ...
        html.Div(
            ...
            children=[
                ...
                dash_molstar.MolstarViewer(
                    id='viewer', style={'width': '700px', 'height':'500px'}
                ),
                ...
                html.Div(
                    [
                        html.Button('Load "From" Ligand', id="load_from_lig", style={"margin-bottom": "10px"}),
                        html.Button('Load "To" Ligand', id="load_to_lig", style={"margin-bottom": "10px"}),
                        html.Button('Load Both Ligands', id="load_both_ligs", style={"margin-bottom": "10px"}),
                    ],
...
def construct_network_graph(highlighted_nodes: list = []):
...
def update_ddg_plot(clickData):
    ...
    fig.add_trace(
        go.Scatter(
            x=[min_val, max_val], y=[min_val, max_val], mode="lines", name="Identity line", line=dict(color="black")
        )
    )
...
def update_network_graph(clickData):
    if not clickData:
        # Return the default network graph if no data is clicked
        return construct_network_graph()

    try:
        # Extracting index from clickData
        index = clickData['points'][0]['customdata']
        data = ddG_df.loc[index]
        lig1, lig2 = data["from"], data["to"]

        # Pass the names of nodes to highlight
        return construct_network_graph(highlighted_nodes=[lig1, lig2])
    except Exception as e:
        print(f"Error updating network graph: {e}")
        # Return the default network graph in case of error
        return construct_network_graph()
...
@app.callback(
    [
        ...
        Output("lig1_img", "src"),
        Output("lig2_img", "src"),
        Output("from-node-storage", "children"),  # Store "from" node
        Output("to-node-storage", "children"),  # Store "to" node
    ],
    [Input("ddg-plot", "clickData")],
)
def update_molecule_display_on_click(clickData):
    ...
    try:
        ...
        return molecule_name, lig1_img, lig2_img, lig1, lig2
    except Exception as e:
        print(e)
        raise PreventUpdate
...
@app.callback(
    ...
    [State('from-node-storage', 'children'),
     ...
     State('both-nodes-storage', 'children')],
    prevent_initial_call=True
)
def display_output(load_from_clicks, load_to_clicks, load_both_ligs, from_node, to_node, both_nodes):
    ...
    # Determine which button was clicked and get the corresponding node
    if button_id == "load_from_lig":
        selected_node = from_node
    elif button_id == "load_to_lig":
        selected_node = to_node
    elif button_id == "load_both_ligs":
        selected_node = [from_node, to_node]
    else:
        raise PreventUpdate
